...
import CartCheckoutVue from "./views/CartCheckout.vue";
...
const routes = [
  ...
  { path: "/cart/checkout", component: CartCheckoutVue, name: "cart-checkout" },
...
router.beforeEach((to, from, next) => {
  ...
  if (isUserAuthenticated && to.matched.some(record => record.meta.isAuth)) {
    ...
    if (!user.isAdmin && (to.matched.some(record => record.meta.onlyAdmin))) {
      next('/client');
      return;
    }
  }
